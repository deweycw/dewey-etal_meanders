...
def write_regions_files(mat_id_1d, cell_id_1d,  face_id_1d, bc_id_1d):
    ...
    np.savetxt('upstream_bc_reg.txt',upstream_bc,delimiter = ' ')
    downstream_bc = np.append(downstream_bc_ids,downstream_bc_faces_1d, axis = 1)
    np.savetxt('downstream_bc_reg.txt',downstream_bc,delimiter = ' ')
    ...
    #print("shale_ids: ", np.shape(shale_ids))

    soil = np.append(soil_ids,soil_faces_1d, axis = 1)
    np.savetxt('soil_reg.txt',soil, delimiter = ' ')
    gravel = np.append(gravel_ids,gravel_faces_1d, axis = 1)
    np.savetxt('gravel_reg.txt',gravel,delimiter = ' ')



    h5_file_name = "../xxgrid010-new.h5"  #'datasets.h5'
    ...
    for top_bc_faces_1d in top_bc_faces_1d_list:
        ...
        print(n)
        filename = f'top_bc_reg_{itop}.txt'
        region_name = filename.split('.')[0]
        print(region_name)
        region = pflo.Region(region_group,region_name,filename)
        region.writeRegion(n)
    ...
    print(n)
    filename = 'downstream_bc_reg.txt'
    region_name = filename.split('.')[0]
    region = pflo.Region(region_group,region_name,filename)
    region.writeRegion(n)
...
if __name__ == "__main__":
    ...
    DEM = np.loadtxt('MZT2.csv',delimiter = ',')
    ...
    write_regions_files(mat_id_1d, cell_id_1d,  face_id_1d, bc_id_1d)

    n_inactive = np.count_nonzero(mat_id_1d == 0)
    n_active = np.count_nonzero(mat_id_1d != 0)

    print(f"Domain dimensions (ny, nx, nz): {ny, nx, nz}")  

    print(f'Total # cells: {ny*nx*nz}')

    print(f"Total # ACTIVE cells: {n_active}")

    print(f"Total # INACTIVE cells: {n_inactive}")
