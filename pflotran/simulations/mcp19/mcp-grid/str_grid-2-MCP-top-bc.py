...
def create_structred_grids(zz, dz, overall_domain_depth, bgs_soil):  #  resolution, zz -- elevation or DEM
    ...
    for ii  in range(0, nx):  
        if ii < 1:
            ...
            face_id[ii,0:bank_top] = 4 # 
            #mat_id[ii,bank_top:np.size(zz_col)] = 0

        # IF TWO SOIL REGIONS ARE DESIRED:
        '''if ii <= 40: 
            fines_id = 2
        else:
            fines_id = 3'''
        
        # IF ONE SOIL REGION IS DESIRED
        fines_id = 2
...
def write_regions_files(mat_id_1d, cell_id_1d,  face_id_1d, bc_id_1d):
    ...
    '''n = np.shape(soil2_faces_1d)[0]
    print(n)
    filename = 'soil2_reg.txt'
    region_name = filename.split('.')[0]
    region = pflo.Region(region_group,region_name,filename)
    region.writeRegion(n)'''
...
if __name__ == "__main__":
    ...
    DEM = np.loadtxt('MCP.csv',delimiter = ',')
