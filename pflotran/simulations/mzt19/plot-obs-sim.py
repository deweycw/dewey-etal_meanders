...
def get_histories(file_location, distances, depths):
    """
    Get simulation histories from PFLOTRAN output file.
    
    Parameters:
    -----------
    file_location : str
        Path to the PFLOTRAN output file
    distances : list
        List of distances to extract data from
    depths : list
        List of depths corresponding to each distance
    ...
    # Determine year from file location for progress tracking
    ...
    print(f"🧪 Found {len(components)} components in simulation data")
    print(f"⏱️  Found {len(times)} time points")
    ...
    print(f"✅ Successfully extracted data for {len(distances)} locations")
    return results_2D, times

def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Plot PFLOTRAN simulation results with observations')
    
    parser.add_argument('file_location', 
                        help='Path to the PFLOTRAN .h5 output file')
    
    parser.add_argument('--year', 
                        choices=[2018, 2019], 
                        type=int,
                        required=True,
                        help='Year of the simulation (2018 or 2019)')
    
    parser.add_argument('--meander', 
                        choices=['MC', 'mc', 'MZ', 'mz'], 
                        required=True,
                        help='Meander type (MC/mc or MZ/mz)')
    
    parser.add_argument('--dim', 
                        choices=['1D', '2D'], 
                        required=True,
                        default='2D',
                        help='1D or 2D model domain')
    
    parser.add_argument('--plot-average', 
                        choices=['True', 'False'], 
                        default='False',
                        help='Whether to plot observation averages (default: False)')
...
if __name__ == "__main__":
    ...
    year = args.year
    meander = args.meander.upper()  # Convert to uppercase for consistency
    plot_average = args.plot_average == 'True'
    dimension = args.dim

    print(f"🚀 Starting PFLOTRAN plotting script")
    print(f"📁 File: {file_location}")
    print(f"📅 Year: {year}")
    print(f"🌊 Meander: {meander}")
    print(f"📊 Plot averages: {plot_average}")
    ...
    if dimension == '1D':
        depths = [0.5, 0.5, 0.5, 0.5, 0.5]

    # Set observation file path (convert meander to lowercase for filename)
    meander_lower = meander.lower()
    chem_f = f'/home/christiandewey/Code/dewey-etal-meanders/data/porewater/{meander_lower}_{year}_porewater.csv'
    
    print(f"📈 Loading observation data from: {chem_f}")
    
    # Load observation data
    try:
        chem_obs = pd.read_csv(chem_f, parse_dates=['Date'])
        print(f"✅ Loaded {len(chem_obs)} observation records")
    except FileNotFoundError:
        print(f"⚠️  Warning: Observation file not found: {chem_f}")
        print("📊 Continuing without observation data")
        chem_obs = None
    except Exception as e:
    ...
    print(f"🎨 Plotting {len(components_to_plot)} components")

    # Process the file
    results, times = get_histories(file_location, distances, depths)
    ...
    for ax, comp in zip(axs, components_to_plot):
        print(f"🖼️  Plotting component: {comp}")
    ...
    # Save the figure
    output_file = file_location.replace('.h5', '.pdf')
