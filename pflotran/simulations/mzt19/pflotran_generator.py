...
Usage:
    python pflotran_generator.py [--year YEAR] [--nx NX] [--upstream_h H] [--downstream_h H]
    
Example:
    python pflotran_generator.py --year 2019 --nx 108 --upstream_h 1.94 --downstream_h 1.66

Author: Christian Dewey
Date: 07.22.2025
Version: 2.1 - Added support for separate CHEMISTRY template file
"""
...
class PFLOTRANGenerator:
    ...
    def __init__(self, year: str, nx: int = 108, upstream_h: float = 1.94, downstream_h: float = 1.66):
        self.year = year
        self.nx = nx
    ...
    def generate_files(self, upstream_file: str = None, downstream_file: str = None) -> Tuple[str, str]:
        """Generate both spin-up and transient input files."""
        ...
        if transient_path.exists():
            ...
            for i, line in enumerate(lines):
                if 'FILENAME' in line and 'restart.chk' in line:
                    lines[i] = f"    FILENAME pflotran-mzt19_{timestamp}_spin-restart.chk\n"
                    break
