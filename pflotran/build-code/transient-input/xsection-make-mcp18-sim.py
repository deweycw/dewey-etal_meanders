...
def write_transient_flow_conditions(nx: int, upstream_file: str, downstream_file:str, save_dir:str):
    ...
    save_dir = save_dir + 'trans-top-bcs/'
...
def assemble_pflotran_input(input_dir: str, save_dir: str, data_dir: str, fname: str, year: str,
                            write_trans_flow: bool, usfile: str = None, dsfile: str = None):

    if write_trans_flow:
        if usfile is None or dsfile is None:
            raise ValueError("usfile and dsfile must be provided when write_trans_flow is True")
        write_transient_flow_conditions(nx=122, upstream_file=usfile, downstream_file=dsfile, save_dir=save_dir)
         
    region_block = write_regions(nx=122)

    flow_conditions_block = write_flow_conditions(nx=122, upstream_h=1.86, downstream_h=0.56, spin=False)
...
if __name__ == '__main__':
    ...
    input_dir = str(SCRIPT_DIR / 'mcp-18') + '/'
    save_dir = str(SCRIPT_DIR / 'mcp-18') + '/'  # Output to same directory
    data_dir = str(SCRIPT_DIR / 'bc_chem_data') + '/'

    # Boundary condition files (relative to build/bcs)
    usfile = str(BUILD_DIR / 'bcs' / 'mc_us_2018_5131h-NODAM.txt')
    dsfile = str(BUILD_DIR / 'bcs' / 'mc_dn_2018_5131h-NODAM.txt')

    fname = 'pflotran-sim-n.in'

    assemble_pflotran_input(input_dir, save_dir, data_dir, fname, '2018',
                            write_trans_flow=True, usfile=usfile, dsfile=dsfile)
