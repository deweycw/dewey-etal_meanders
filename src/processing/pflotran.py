...
# Add pflotran-utils to path if not installed
_PFLOTRAN_UTILS_PATH = Path('/Users/christiandewey/Code/pflotran-utils')
if _PFLOTRAN_UTILS_PATH.exists() and str(_PFLOTRAN_UTILS_PATH) not in sys.path:
    sys.path.insert(0, str(_PFLOTRAN_UTILS_PATH))

from pflotranutils.h5_output.calc.cross_section import CrossSection

# Project root directory (two levels up from this file: src/processing/ -> project root)
...
def _resolve_path(relative_path: str, search_paths: Optional[List[str]] = None) -> Path:
    """
    ...
    for search_dir in search_paths:
        candidate = _PROJECT_ROOT / search_dir / relative_path
        if candidate.exists():
            return candidate
    ...
    searched = [str(_PROJECT_ROOT / sp / relative_path) for sp in search_paths]
...
class PflotranProcessor(CrossSection):
    ...
    # Class-level project root for reference
    ...
    def __init__(self, h5_path: Optional[str] = None, meander: str = 'MZ',
        ...
        if h5_path is not None:
            self._h5_path_resolved = _resolve_path(h5_path)
    ...
    def h5_path(self, value: Optional[str]):
        ...
        if value is not None:
            self._h5_path_resolved = _resolve_path(value)
        else:
