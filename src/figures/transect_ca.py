...
# %%
def load_ca(fname, positions, ca_col='Ca', ca_scale=1.0):
    """Load porewater CSV and extract Ca at mapped transect positions."""
    ...
    sub['Ca_mM'] = sub[ca_col] * ca_scale
    sub = sub[sub['Ca_mM'].notna()]
    return sub
...
def make_site_panel(site_data, years, catpos, ax, title):
    ...
    if len(years) == 1:
        sns.boxplot(
            x='trans_pos_m', y='Ca_mM', data=site_data, ax=ax,
            order=label_order,
    ...
    for year, offset, pal in zip(years, offsets, palettes):
        ...
        sns.scatterplot(
            data=df, x='CatPos', y='Ca_mM', hue='Date',
...
def make_combined_panel(data, ax):
    ...
    sns.boxplot(
        x='location', y='Ca_mM', data=data, ax=ax,
...
# ── Load data ──
# MZA: Ca in M → convert to mM
mza18 = load_ca('porewater/mz_2018_porewater.csv', mza_positions,
                 ca_col='Ca', ca_scale=M_TO_MM)
mza19 = load_ca('porewater/mz_2019_porewater.csv', mza_positions,
                 ca_col='Ca', ca_scale=M_TO_MM)

# MCP: Ca_M in M (ess-dive) or Ca in M (2019) → convert to mM
mcp17 = load_ca('porewater/mc_2017_porewater_ess-dive.csv', mcp_positions,
                 ca_col='Ca_M', ca_scale=M_TO_MM)
mcp18 = load_ca('porewater/mc_2018_porewater_ess-dive.csv', mcp_positions,
                 ca_col='Ca_M', ca_scale=M_TO_MM)
mcp19 = load_ca('porewater/mc_2019_porewater.csv', mcp_positions,
                 ca_col='Ca', ca_scale=M_TO_MM)


# %%
# ── Build combined dataframes for 2018 and 2019 ──
mcp18_c = mcp18.copy()
mza18_c = mza18.copy()
mcp18_c['site'] = 'MCP'
mza18_c['site'] = 'MZA'
...
mcp19_c['site'] = 'MCP'
mza19_c['site'] = 'MZA'
combined_2019 = pd.concat([mcp19_c, mza19_c], ignore_index=True)
combined_2019['location'] = combined_2019['trans_pos_m'].map(loc_map)
combined_2019['CatPos'] = combined_2019['location'].map(combined_catpos)

# ── Two-panel figure: 2018 (left) + 2019 (right) ──
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7, 3.5))

make_combined_panel(combined_2018, ax1)
ax1.set_title('2018', fontsize=10)
ax1.text(XPLTL, YPLTL, 'a', transform=ax1.transAxes, fontsize=10, fontweight='bold')

make_combined_panel(combined_2019, ax2)
ax2.set_title('2019', fontsize=10)
ax2.text(XPLTL, YPLTL, 'b', transform=ax2.transAxes, fontsize=10, fontweight='bold')
...
# Single set of colorbars (MCP viridis + MZA rocket) on the right
for pal, label, y0 in [('viridis', 'MCP', 0.55), ('rocket', 'MZA', 0.15)]:
    cbar_ax = fig.add_axes([1.00, y0, 0.015, 0.3])
    mind, maxd = date(2018, 5, 25), date(2018, 10, 25)
    mindf, maxdf = mind.timetuple().tm_yday, maxd.timetuple().tm_yday
    norm = matplotlib.colors.Normalize(vmin=mindf, vmax=maxdf)
    sm = cm.ScalarMappable(norm=norm, cmap=pal)
    sm.set_array(np.arange(mindf, maxdf))
    ...
    labels = [timedelta(days=float(d) - 1.0) + startdate for d in cb_labels]
    cb.ax.set_yticklabels([d.strftime("%-d %b") for d in labels], size=7)
    cb.ax.set_title(label, size=8, rotation=0)

plt.savefig(svdir / 'ca_transect_2018_2019.png', bbox_inches='tight', dpi=200)
